    ...
    data.forEach( product => {
      ...
      const productDiv = `
            <div class="bg-gray-800 rounded-lg flex flex-col items-center justify-center w-full max-w-lg mx-auto"> 
              <img class="w-55 h-64 mx-auto rounded-lg mb-4 shadow-lg shadow-teal-500" src="../public/${images[0] || 'default-image.png'}" alt="${product.title}">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-2">
                        <h2 class="block text-xl font-semibold text-center text-white">${product.title}</h2>
                        <p class="block text-gray-300 text-sm text-center">$${product.value.toLocaleString()}</p>
                    </div>
